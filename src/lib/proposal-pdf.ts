...
export function buildProposalPDFData(
  ...
  return {
    ...
    valor_global: parseFloat(fd.valor_global) || 0,
...
body{font-family:Arial,Helvetica,sans-serif;color:#333;line-height:1.6;font-size:13px}
.header{display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #288f87;padding-bottom:12px;margin-bottom:20px}
.header img{height:60px;object-fit:contain}
.header h1{color:#288f87;font-size:20px;margin:0;text-align:right}
.section{margin-bottom:18px}
.section h2{color:#288f87;font-size:15px;border-bottom:1px solid #ddd;padding-bottom:4px;margin:0 0 6px}
...
.info-grid div strong{color:#288f87}
table{width:100%;border-collapse:collapse;margin:8px 0}
th{background:#288f87;color:#fff;padding:8px;text-align:left;font-size:13px}
td{padding:8px;border:1px solid #ddd;font-size:13px}
td:last-child,th:last-child{text-align:right}
.signature{margin-top:50px;text-align:center}
.signature-line{border-top:1px solid #333;width:280px;margin:40px auto 5px}
.footer{margin-top:40px;padding-top:12px;border-top:2px solid #288f87;text-align:center;font-size:11px;color:#777}
</style></head><body>
<div class="header"><img src="${esc(logo)}" alt="Logo"><h1>PROPOSTA COMERCIAL</h1></div>
<div class="info-grid">
...
<div class="section"><h2>8. Investimento</h2>
<table><thead><tr><th>Modalidade</th><th>Valor</th></tr></thead><tbody>
<tr><td>Módulo 4 horas</td><td>${fmtCurrency(data.valor_modulo_4h)}</td></tr>
<tr><td>Módulo 8 horas</td><td>${fmtCurrency(data.valor_modulo_8h)}</td></tr>
<tr><td>Valor Global</td><td>${fmtCurrency(data.valor_global)}</td></tr>
</tbody></table>
${data.condicoes_pagamento ? `<p><strong>Condições de Pagamento:</strong> ${esc(data.condicoes_pagamento)}</p>` : ''}
</div>
...
<div class="section"><h2>10. Perfil do Instrutor</h2><p>${esc(data.perfil_instrutor)}</p></div>
<div class="footer">
