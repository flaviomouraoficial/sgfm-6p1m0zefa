...
const generateQuestions = () => {
  ...
  for (const dim of DIMENSIONS) {
    for (let i = 0; i < QUESTIONS_PER_DIM; i++) {
      qs.push({
        id: `q${id++}`,
        dimension: dim,
        text: `Como você avalia o aspecto ${i + 1} de ${dim} na sua empresa?`,
      })
    }
  }
  return qs
...
export default function AssessmentFlow() {
  ...
  const finishAssessment = async () => {
    ...
    try {
      ...
      await pb.collection('v1_saas_results').update(id!, {
        ...
        status: 'Concluído',
        completed_at: new Date().toISOString().replace('T', ' ').substring(0, 19) + 'Z',
      })
  ...
  if (!result) return <div className="p-8">Carregando...</div>
