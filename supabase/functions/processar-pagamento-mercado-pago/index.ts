...
Deno.serve(async (req) => {
  ...
  if (!token) {
    ...
    return new Response(JSON.stringify({ error: 'Credenciais de pagamento não configuradas no servidor.' }), {
  ...
  try {
    ...
    if (planoError || !plano) {
      throw new Error(`Erro ao buscar o pacote selecionado: ${planoError?.message || 'Pacote inexistente'}`);
    }
    ...
    if (!mpResponse.ok || !preferenceResult.id) {
      ...
      throw new Error(`Falha de comunicação com o Mercado Pago: ${preferenceResult.message || 'ID não retornado'}`);
    }
