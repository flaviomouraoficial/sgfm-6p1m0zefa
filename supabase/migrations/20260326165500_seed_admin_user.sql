...
DECLARE
  new_user_id uuid;
BEGIN
  -- Seed requested admin user (usuario: admin, senha: admin)
  ...
  ELSE
    -- Garante que a senha seja atualizada para 'admin' caso o usuário já exista
    UPDATE auth.users 
    SET encrypted_password = crypt('admin', gen_salt('bf'))
    WHERE email = 'admin@grupoflaviomoura.com.br';
  END IF;

  -- Seed default system owner
